...
<title>View Recruiter List</title>
...
<body>
			...
			<div class="table-wrapper">
				...
				<div class="table-title">
					<div class="row">
						<div class="col-sm-6">
							<h2>
								<font color="#836ea7">View Recruiter List</font>
							</h2>
						</div>
					</div>
				</div>
				<div class="p-3 table-responsive">
					<table class="table table-striped table-hover table-bordered"
						...
						<tbody>
							<c:forEach items="${RecruiterListBean.frmRecruiterList}"
								var="list">
								<c:set var="status" value="${list.status.description}" />
								<tr>
									...
									<c:if test="${status =='ACTIVE'}">
										<td><a
											href="deactivateRecruiterAccount.do?frmRecruiterId=${list.id}"
											...
											data-confirm="Are you sure you want to deactivate this account ?">
												<i class="zmdi zmdi-alert-octagon"></i>Deactivate</a></td>
									</c:if>
									<c:if test="${status =='INACTIVE'}">
										<td><a
											href="activateRecruiterAccount.do?frmRecruiterId=${list.id}"
	...
	<!-- for data table -->
	<jsp:include page="/WEB-INF//jsp/inc/data_table_col6.jsp"></jsp:include>
	
	<!--show success message-->
	<c:if test="${RecruiterListBean.successMsg!=null}">
		<script type="text/javascript">
			swal("Success!", "${RecruiterListBean.successMsg}", "success");
		</script>
	</c:if>
</body>
</html>
